...
# 로비 페이지에서 사용하는 WebSocket Consumer입니다.
# 서버가 보낸 이벤트를 프론트가 그릴 수 있는 JSON 형태로 중계합니다.
#
...
class LobbyConsumer(AsyncWebsocketConsumer):
    # 소켓 연결 시 "lobby" 그룹에 현재 클라이언트 채널을 등록합니다.
    async def connect(self):
        # group_add("lobby", channel_name): "lobby" 브로드캐스트를 이 클라이언트가 받게 함
        await self.channel_layer.group_add("lobby", self.channel_name)
        await self.accept()

    # 브라우저가 떠나면 그룹에서 채널을 제거해 누수/중복 전송을 방지합니다.
    async def disconnect(self, close_code):
        await self.channel_layer.group_discard("lobby", self.channel_name)

    # 파티 카드 생성/수정 이벤트를 그대로 브라우저로 전달합니다.
